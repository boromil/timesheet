(function () {
  ...
  Vue.component("InputErrors", {
    template:
      '<div><div v-for="e in errors" class="form-control-feedback">{{ e }}</div></div>',
    props: {
      errors: {
        type: Array,
        default: function () {
  ...
  Vue.component("NewTimesheet", {
    ...
    methods: {
      ...
      localIsFormValid: function () {
        ...
        if (
          !checkIfValid(isNaN(to), this.durationTo.errors, dateTimeInvalidMsg)
        ) {
          return false;
        }
